...
//      if true, it will only place jumpers on the first 4 rows of pins,
//      which can't be reversed in firmware,
//      i.e. RAW and P1, GND and P0, GND and RST, GND and VCC.
...
//      allows to define the size of the via.
//        Not recommended below 0.56 (JLCPCB minimum),
//        or above 0.8 (KiCad default), to avoid overlap or DRC errors.
//    via_drill: default is 0.4
//      allows to define the size of the drill.
//        Not recommended below 0.3 (JLCPCB minimum),
...
//  - Leave more space between the vias to allow easier
//      routing through the middle of the footprint
//
...
//  - Add ability to only have required jumpers
//      and let the rest be handled in firmware
//
...
// # Further credits
//
// This footprint was created from scratch by @infused-kim,
// but is based on the ideas from these footprints:
//
  ...
  body: (p) => {
    ...
    const gen_socket_row = (
      ...
      let socket_row_base = `
        ...
        -12.7 + row_offset_y
      } ${p.rot}) (size 1.7 1.7) (drill 1) (layers *.Cu *.Mask) ${
      ...
      let socket_row_vias = `
        ${"" /* Inside VIAS */}
        (pad ${via_num_right} thru_hole circle (at 3.4 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size ${p.via_size} ${p.via_size}) (drill ${
        p.via_drill
      }) (layers *.Cu *.Mask) ${net_right})
        (pad ${via_num_left} thru_hole circle (at -3.4 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size ${p.via_size} ${p.via_size}) (drill ${
        p.via_drill
      ...
      let socket_row_rectangular_jumpers = `
        ...
        (pad ${socket_hole_num_left} smd rect (at -5.48 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size 0.6 1.2) (layers F.Cu F.Mask) ${
        p.local_net(socket_hole_num_left).str
      })
        (pad ${via_num_left} smd rect (at -4.58 ${-12.7 + row_offset_y} ${
        p.rot
      }) (size 0.6 1.2) (layers F.Cu F.Mask) ${net_left})

        ${"" /* Jumper Pads - Front Right */}
        (pad ${via_num_right} smd rect (at 4.58 ${-12.7 + row_offset_y} ${
        p.rot
      }) (size 0.6 1.2) (layers F.Cu F.Mask) ${net_right})
        (pad ${socket_hole_num_left} smd rect (at 5.48 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size 0.6 1.2) (layers F.Cu F.Mask) ${
        p.local_net(socket_hole_num_right).str
        ...
        (pad ${socket_hole_num_left} smd rect (at -5.48 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size 0.6 1.2) (layers B.Cu B.Mask) ${
        p.local_net(socket_hole_num_left).str
      })
        (pad ${via_num_right} smd rect (at -4.58 ${-12.7 + row_offset_y} ${
        p.rot
      }) (size 0.6 1.2) (layers B.Cu B.Mask) ${net_right})

        ${"" /* Jumper Pads - Back Right */}
        (pad ${via_num_left} smd rect (at 4.58 ${-12.7 + row_offset_y} ${
        p.rot
      }) (size 0.6 1.2) (layers B.Cu B.Mask) ${net_left})
        (pad ${socket_hole_num_left} smd rect (at 5.48 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size 0.6 1.2) (layers B.Cu B.Mask) ${
        p.local_net(socket_hole_num_right).str
      ...
      let socket_row_chevron_jumpers = `
        ...
        -12.7 + row_offset_y
      } ${p.rot}) (size 0.2 0.2) (layers F.Cu F.Mask) ${
        p.local_net(socket_hole_num_left).str
      }
            (zone_connect 2)
        ...
        180 + p.rot
      }) (size 0.2 0.2) (layers F.Cu F.Mask) ${net_right}
            (zone_connect 2)
            (options (clearance outline) (anchor rect))
            (primitives
              (gr_poly (pts
        ...
        -12.7 + row_offset_y
      } ${180 + p.rot}) (size 0.2 0.2) (layers F.Cu F.Mask) ${
        p.local_net(socket_hole_num_right).str
      }
            (zone_connect 2)
            (options (clearance outline) (anchor rect))
          ...
          (pad ${socket_hole_num_left} smd custom (at -5.5 ${
        -12.7 + row_offset_y
      } ${p.rot}) (size 0.2 0.2) (layers B.Cu B.Mask) ${
        p.local_net(socket_hole_num_left).str
      }
            (zone_connect 2)
            (options (clearance outline) (anchor rect))
        ...
        p.rot
          ...
          (pad ${via_num_left} smd custom (at 4.775 ${-12.7 + row_offset_y} ${
        180 + p.rot
      }) (size 0.2 0.2) (layers B.Cu B.Mask) ${net_left}
            (zone_connect 2)
            (options (clearance outline) (anchor rect))
            (primitives
              (gr_poly (pts
          ...
          (pad ${socket_hole_num_right} smd custom (at 5.5 ${
        -12.7 + row_offset_y
      } ${180 + p.rot}) (size 0.2 0.2) (layers B.Cu B.Mask) ${
        p.local_net(socket_hole_num_right).str
      }
            (zone_connect 2)
            (options (clearance outline) (anchor rect))
      ...
      if (show_silk_labels == true) {
        socket_row += `
          ...
          row_num < 4 || !p.only_required_jumpers ? 3 : 6
        } ${-12.7 + row_offset_y} ${p.rot}) (layer F.SilkS)
          ...
          row_num < 4 || !p.only_required_jumpers ? 3 : 6
        } ${-12.7 + row_offset_y} ${180 + p.rot}) (layer B.SilkS)
    ...
    const common_top = `
        (module "ceoloide:nice_nano" (layer F.Cu) (tedit 6451A4F1)
          ...
          (fp_text reference "${p.ref}" (at 0 -15 ${p.rot}) (layer F.SilkS) ${
      p.ref_hide
    }
